...
function queryWithDomainOperators(request: SearchRequest): string {
...
export class BraveSearchProvider implements SearchProvider {
  ...
  async search(request: SearchRequest, signal: AbortSignal): Promise<SearchResult> {
    ...
    url.searchParams.set("q", queryWithDomainOperators(request));
    ...
    const values =
      raw &&
      typeof raw === "object" &&
      (raw as { web?: unknown }).web &&
      typeof (raw as { web: unknown }).web === "object"
    ...
    if (!Array.isArray(values)) {
      throw new GroundlaneError(
        "UPSTREAM_ERROR",
        "search",
        "Brave returned a malformed response",
        true,
      );
    }
