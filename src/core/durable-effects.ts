...
export type EffectRetryResult =
  | { readonly status: "claimed"; readonly effect: DurableEffect; readonly revision: number }
  | { readonly status: "blocked"; readonly effect: DurableEffect; readonly revision: number };
...
function decode(record: DurableRecord): DurableEffect {
  ...
  if (
    ...
    !["provider_task_create", "provider_task_update", "provider_task_cancel", "paid_upstream_call", "artifact_write"].includes(item.effectKind ?? "") ||
...
export class DurableEffectJournal {
  ...
  async retry(
    identity: { jobId: string; effectKind: DurableEffectKind; operationKey: string },
    expectedRevision: number,
    nowMs: number,
  ): Promise<EffectRetryResult> {
    if (identity.effectKind !== "provider_task_cancel") {
      throw new Error("only provider cancellation effects may be retried");
    }
    const key = keyFor(identity.jobId, identity.effectKind, identity.operationKey);
    const current = await this.store.get(key);
    if (current === null) throw new Error("durable effect claim is missing");
    const previous = decode(current);
    // Older document runtimes recorded an explicitly unacknowledged cancel as
    // succeeded. This receipt is not proof of cancellation and remains retryable.
    const retryable = previous.status === "uncertain" ||
      (previous.status === "succeeded" && previous.receipt === "not-acknowledged");
    if (current.revision !== expectedRevision || !retryable) {
      return { status: "blocked", effect: previous, revision: current.revision };
    }
    const next: DurableEffect = {
      ...previous,
      status: "claimed",
      updatedAt: nowMs,
      receipt: null,
    };
    const result = await this.store.compareAndSwap(key, expectedRevision, {
      value: encode(next),
      nowMs,
    });
    if (result.status === "updated") {
      return { status: "claimed", effect: next, revision: result.record.revision };
    }
    if (result.status === "missing") throw new Error("durable effect claim is missing");
    return {
      status: "blocked",
      effect: decode(result.record),
