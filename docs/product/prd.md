...
Groundlane 是供 AI agent 使用的 vendor-neutral **trusted content access layer**。產品方向是透過一個 endpoint 安全取得並解析 Web 與文件內容，讓來源、execution path 與 provider provenance 可追蹤。現有 runtime 已實作 Web search/retrieval、deterministic extraction、URL/raw HTML parsing，以及第一個有界同步 `document_parse` slice；OCR、durable artifact/cache/async 與完整 release gates 仍在 Phase 3 roadmap。目前 operator 可使用 Groundlane 自有 retrieval/parser/browser engines，並在內建 provider catalog 中設定一家或多家 managed providers 作混合 routing；operator-hosted provider registration 是後續產品目標，不是現有 runtime configuration。核心 Web primitives 包含 `web_fetch`、`web_search`、`web_answer`、`web_research`、`web_content`、`web_map`、`web_crawl`、`web_news`、`web_images`、`web_extract` 與相容的 HTML `parse`。部署也提供 `provider_balance`、`provider_capabilities`、`provider_quota` 與 `search_budget_status` 等 diagnostics。
...
- legacy MCP contract tests 能完成 `initialize`、`tools/list` 與公開 `tools/call`；MCP `2026-07-28` 的無 handshake request、`server/discover` 與 per-request capability negotiation 仍待 migration acceptance。
...
| Scoped/operator-owned corpus | 已有獨立 corpus lifecycle 與 `corpus_search` tool family，不混入 `web_search` | Self-hosted SQLite durable manifest/artifact/derived-index adapter 已接 production composition；Cloudflare managed index、外部 provider、operator-hosted enterprise/private search仍需各自通過 gate |
...
| D1 | managed-token registry、MCP Tasks metadata、source upload/artifact lifecycle與 document cache metadata已掛 Worker runtime；皆有 deterministic adapter/composition tests，controlled deployment gate仍開放 | 可查詢的完整 document-job metadata、status history、billing provenance、retention、artifact index，以及通過 gate 後的 Cloudflare corpus lifecycle metadata adapter | raw bearer token、大型頁面內容、hot concurrency lock、最新 quota 判斷的唯一同步機制、未受 session/primary constraint 保護的 authorization read；derived index不得取代 corpus manifest/deletion truth |
| R2 | source upload staging/final artifact與 document-cache immutable payload已掛 Worker runtime；controlled real-R2 smoke仍開放 | Cloudflare durable async output、corpus source artifacts、crawl results、screenshots/traces與 benchmark artifacts | state machine、queue、distributed lock、quota counter、corpus identity/ACL truth；不得讓 public contract依賴 R2 object key |
...
- [x] Phase 0/MVP legacy client 可 `initialize`、list tools，且只看到已註冊的 Groundlane stateless tools 與 provider diagnostics；這是截至 `2025-11-25` protocol family 的 compatibility baseline，不代表已符合 MCP `2026-07-28`。未來 stateful tool family 啟用時必須另有 capability/version gate 與 schema snapshot。
...
- [x] 凍結目前 `@modelcontextprotocol/sdk` v1／`2025-11-25` compatibility baseline：保存 initialize、tools/list、tools/call、auth challenge、error、schema 與 transport fixtures；明定 legacy coexistence、退場條件與 rollback，不用「stateless mode」推論新版 wire compatibility。
- [x] 將 MCP runtime 從 v1 單體套件遷移到官方 v2 分拆套件；先用官方 codemod 產生可審查 diff，再逐一處理 imports、transport middleware、handler context、auth、schema、behavioral changes 與現有 `Transport` double cast。SDK 升級本身不算 protocol acceptance。
- [x] 建立 MCP `2026-07-28` modern handler：支援無 `initialize`／無 `Mcp-Session-Id` 的 self-contained request、per-request client info/capabilities、`server/discover` 與 request-scoped server factory；legacy `2025-11-25` 必須走明確、可測、可關閉的 compatibility path，不得由模糊 fallback 猜測 request era。
- [x] 在 Worker、Container 與 MCP transport 邊界驗證 `MCP-Protocol-Version`、`Mcp-Method`、`Mcp-Name` 和 JSON-RPC body 一致性；header 只供 routing、metering 與早期 policy，不得取代 body validation。保留既有 bearer/OAuth、signed internal principal、credential binding、deadline、cancellation、rate limit 與 secret-redaction 邊界。
- [x] 為 tools/prompts/resources list 與 resource read 實作新版 deterministic ordering、`ttlMs`、`cacheScope` 與 principal/capability-aware cache partition；動態 availability、credential 或 policy 改變時不得把跨 caller 的舊 capability catalog 當成有效結果。
- [x] 實作並測試新版 Multi Round-Trip Requests：`input_required`、`inputResponses`、opaque `requestState` 的完整性、owner binding、expiry、重送與跨 instance resume；server-to-client request 只能由進行中的 caller request 觸發，不得依賴長開 SSE 或 unsolicited prompt。2026 HTTP cancellation 以關閉該 request stream 傳遞，不能混同 task cancellation。
- [x] 將 tool `inputSchema`／`outputSchema` 驗收到完整 JSON Schema 2020-12 contract，保存 schema snapshots，限制 schema depth、validation time 與 external `$ref`；同步核對新版 resource-not-found error、任意 JSON `structuredContent` 與 legacy client tolerance。
- [x] 逐條核對 MCP `2026-07-28` authorization hardening：issuer/resource metadata、RFC 9207 `iss`、client metadata document 與現有 DCR compatibility。Groundlane 的 data-plane principal、admin credential、Worker-to-Container internal context 與 provider secrets 必須維持分離；只完成 OAuth happy path 不算通過。
- [x] 將 MCP Tasks 視為獨立、雙方 opt-in 的正式 extension：只在 client per-request capabilities 宣告 `io.modelcontextprotocol/tasks` 時由 server-directed call 回 task handle，實作 durable `tasks/get`、`tasks/update`、`tasks/cancel`、terminal monotonicity、TTL 與 reconnect；不得沿用已退役的 experimental Tasks shape，也不得提供不安全的全域 `tasks/list`。不支援 extension 的 client 保留明確 start/status/result/cancel compatibility tools。
- [x] R2 upload、ArtifactRef 與 document processing 的 core service 保持 protocol-neutral；modern/legacy MCP adapters 共用相同 ownership、credential binding、idempotency、expiry、immutable finalize 與 stable errors。Presigned PUT 是 Groundlane 的 upload handoff，不宣稱為 MCP core file-upload capability。
- [ ] 建立可重跑的 conformance 與 target-client matrix：至少涵蓋 legacy/modern protocol detection、discover、tools list/call、structured content、cache hints、MRTR、auth、disconnect/cancel，以及 Claude、Codex、Cursor 對 Tasks create/get/update/cancel/reconnect 與 upload handoff 的實際支援。SDK types、local mocks、成功 `tools/list` 或 fallback client 都不能替代 raw transcript／controlled live evidence。
- [ ] 同步 README、README.zh-TW、configuration、deployment、security、release notes 與 migration/rollback runbook；依序通過 deterministic suite、官方 conformance suite、controlled Cloudflare deployment smoke 與 target-client live smoke，才可宣稱 Groundlane 支援 MCP `2026-07-28`。legacy compatibility 與 production enablement 必須分開回報。
...
- [ ] 實作官方 MCP Tasks extension 與 explicit fallback async tools 共用的完整 runtime，並通過上方 MCP `2026-07-28` negotiation／target-client gates。Lifecycle 涵蓋 caller ownership binding、TTL/expiry、provider job mapping（不暴露 provider job ID）、credential binding、status monotonicity、billing provenance與 sanitized errors。三種 cancel（caller 停止等待、Groundlane 取消 polling、upstream 真正取消）必須是獨立狀態，沒有 provider acknowledgment 不得回報 upstream 已取消。實作須符合 `src/core/async-lifecycle.ts` contract 規格。
...
- [x] 實作 durable crawl job runtime：provider-neutral create/status/result/cancel endpoint、owner binding、credential binding、expiry、pagination、partial result、total page/byte/output budgets 與 upstream cancel evidence。不暴露 provider job ID。Caller cancellation、Groundlane polling 停止與 upstream crawl 真正取消必須是獨立狀態；沒有 provider acknowledgment 不得回報 upstream 已取消。Owner mismatch、expired/unknown job 與 sanitized upstream failures 有 deterministic integration tests。實作須符合 `src/core/async-lifecycle.ts` 中 durable crawl contract 規格。
...
- [ ] 完成 document processing MCP acceptance。第一個 bounded synchronous `document_parse` slice 已掛載：inline canonical base64 與經既有 SSRF/fetch policy 的 public URL 可進入 deterministic text-PDF、OOXML、structured-text、ODF/RTF/EPUB/EML profiles，並輸出 canonical envelope/projection。不接受 caller path 或 storage coordinate。完整設定的 Cloudflare edge profile 也可讀取 verified source `ArtifactRef`，經 body-bound internal bridge 交給同一 parser；OCR、完整 security fixtures、target-client compatibility、controlled deployment 與 production smoke 尚未完成。
- [ ] 實作 Cloudflare upload flow runtime（create intent → direct R2 PUT → complete/verify/finalize → process），接上 R2 storage。Create intent 限制 content length/MIME/expiry/ownership/single PUT；complete 驗證 size/MIME sniff/content hash/ownership 再 promote 到 immutable final object。`ArtifactRef` 在 finalize 成功後產生，是 storage-neutral opaque identity（不暴露 R2 key/presigned URL/filesystem path）。Upload intent default 15 分鐘、artifact default 24 小時，effective TTL 受 deployment/operator cap 約束。過期/超限/mismatch/重放/跨 ownership 有 integration tests。第一版拒絕 multipart。實作須符合 `src/core/upload-intent.ts` contract 規格。目前已有 D1/R2、SigV4 single-PUT handoff、verify/finalize/process、operator TTL cap 與 scheduled cleanup 的 deterministic runtime tests；controlled deployment、真實 R2 與 target-client smoke 尚未完成。
- [ ] 實作 artifact processing 與 lifecycle runtime：只讀 verified object、成功不自動延長 retention 或 enroll corpus。Cancel/failure/expiry/explicit delete/orphan cleanup 實際執行 source artifact/output/remnants/corpus state 的處理。Staging cleanup window ≤ 1 小時，區分 logically expired 與 physical cleanup pending。Explicit delete 立即撤銷 ArtifactRef access。Upload handoff 逐一驗證 Claude/Codex/Cursor 相容性；不相容 client 走 CLI/dashboard。目前 verified source read、processing bridge、public `document_artifact_delete`、CAS-first logical revocation、cache-binding revocation、finalizing orphan cleanup、R2 staging revalidation、hourly bounded cleanup與 self-host durable corpus lifecycle 已有 deterministic tests；完整 cancel/failure propagation、durable output lifecycle、controlled deployed cleanup 與 client 驗證未完成。
- [ ] 實作 document processing cache runtime（接上 D1/KV），working default 24 小時。三種 cache mode（use/refresh/bypass）實際運作：use hit-read/miss-execute-and-write、refresh skip-read/re-execute-and-replace、bypass no-read/no-write。Cache key 以 content hash + engine/model/schema version 組成，禁止只用 URL/filename。Source binding 隔離：刪除一個 source 只撤銷該 binding，不影響同 hash 其他 source。Cache hit 回 `cached=true` 與完整 provenance，不偽裝成新 execution。Cache failure 不使 processing fail。不改變現有 `web_fetch`/`web_extract`/`parse` contract。實作須符合 `src/core/document-cache-contract.ts` contract 規格。目前 self-hosted SQLite 與 Cloudflare D1/R2 都已接上正式 `document_parse` composition；Cloudflare 透過 versioned、body-bound private lookup/commit bridge，TTL policy 留在 Worker，並有 credential-scoped binding、artifact-expiry cap、bounded cleanup 與 deterministic edge-composition tests。Public artifact delete與 expiry cleanup 已透過 reverse source index 撤銷所有 parser-option bindings；corpus source 已接 document cache identity 與 signed revoke，2026-09-05 [isolated staging 的 9 個 inline miss/hit/rebind/refresh/bypass 檢查已通過](../verification/staging-cache-2026-09-05.json)，並修正 Container SDK private outbound handler 註冊；verified-source upload/delete 與實際排程 cleanup smoke 仍未完成。
- [ ] 完成 canonical document envelope runtime acceptance。現有同步 slice已輸出 versioned blocks/spans/capabilities/provenance、deterministic projection，cache hit 也會依本次 invocation 重建 source binding。Self-hosted inline/URL 已可透過 `DOCUMENT_ARTIFACT_STATE_PATH` 把超限結果保存為 durable output `ArtifactRef`，支援有界分段讀取、restart、tenant/credential isolation、expiry 與 retryable source/result cleanup；Cloudflare result storage 已有 private bridge、deterministic 與本機 workerd 證據。15 種代表格式已通過 canonical wire 與四種 projection roundtrip，並強制 reading order 是 blocks 的完整排列；controlled deployment 與 client schema compatibility 仍未完成。
- [ ] 實作 sync/async 雙軌 document execution runtime。現有同步 slice不 silent 轉 async；provider-neutral durable runtime 與 Reducto candidate adapter 已有 deterministic contracts，包含 acknowledged task/result receipt 恢復、取消／過期後可重試清理及 public status credential redaction。已有 CAS-leased dispatcher、native D1/R2 composition、output write reservation 的 pre-ack 恢復與同步／非同步共用 envelope/projection；本機 workerd 已驗證 native storage lifecycle（provider transport 為 injected fixture）。Submit 於排程持久化後才 acknowledged；job-owned immutable source snapshot、source-guarded atomic metadata/outbox admission，以及 source delete/expiry 對 queued/completed job 的立即撤銷已有 deterministic restart/race tests。Explicit fallback `document_job_create/status/cancel` 已掛 MCP inventory 與 authenticated Worker execution，每分鐘 bounded dispatcher 與每小時 cleanup 採 exact cadence 分流；flag 與 D1/R2/secret/provider prerequisite 不完整時 fail closed。官方 Tasks mapping／negotiation、controlled provider 與 target-client acceptance 仍未完成。
- [x] 實作 corpus lifecycle runtime（create/enroll/update/remove/status/search/delete），接上 durable backend adapter。Self-hosted production composition 在 `CORPUS_STATE_PATH` 設定後使用 revision-fenced SQLite manifest truth、immutable normalized source artifacts與可 exact rebuild 的 derived index；restart、ACL、credential/tenant isolation、retention/expiry、remove tombstone、delete revoke/cleanup retry與真實 MCP composition integration tests 已通過。Cloudflare corpus backend仍需獨立 gate，不包含在本項 self-host runtime 證據。
- [x] 實作 `document_policy` capability view（MCP tool 或 resource），公告 cache/upload/artifact/corpus defaults 與 hard caps，各 response 回 effective absolute expiry。相對/絕對 expiry 互斥，超界回 validation error 不 clamp。
- [x] 實作 durable async retry idempotency guard runtime，防重複 provider task creation、付費 upstream call 與 artifact writes。Durable `ArtifactRef` storage-neutral 並含 tenant/owner binding、content hash、retention/deletion policy，Cloudflare deployment 映射到 R2。沒有 provider acknowledgment 不宣稱 upstream cancel 成功。
- [ ] 現有 URL/raw-HTML `parse` flat schema 保持相容；新 document tool 使用 canonical envelope。既有 schema snapshot/tool-list/HTML tests 已過，Claude/Codex/Cursor compatibility 尚待實測。
...
- [x] 每個 stateful resource 必須分別定義 owner、credential binding、TTL/retention、status/result/cancel、notification/webhook、abuse controls、quota/billing provenance、data deletion 與 deterministic lifecycle tests。
...
- [x] Authenticated session 第一版若獲核准，只接受 allowlisted domain、operator-authorized account、human-in-the-loop login/MFA、provider-owned opaque profile/context reference、principal/site/account binding、exclusive lease、idle/absolute TTL、explicit release/delete 與 read-only bounded navigation。不得保存 raw password、TOTP seed、passkey、可匯出的 cookie/storage snapshot，也不得承諾 CAPTCHA 或 managed-challenge bypass。

### Security 與 deployment

- [x] Static bearer、D1 managed token 與 OAuth access token 都轉成同一個 `AuthenticatedPrincipal` contract；V1 `principalId=owner` 不被宣稱為 user/tenant isolation，OAuth `clientId` 只作 client/grant attribution；caller-provided principal/tenant/policy headers 被忽略或拒絕，不能覆寫驗證結果。
- [x] `GROUNDLANE_AUTH_TOKEN` 只作 legacy/local/backward-compatible data-plane auth，不能存取 credential-management operation。新增 `GROUNDLANE_ADMIN_TOKEN` 只作 credential bootstrap/recovery/admin auth且不能呼叫 `/mcp`；兩者和 owner passphrase、internal-signing secret、provider secrets皆不得共用。
- [x] `GROUNDLANE_ADMIN_TOKEN` 對 `/mcp`、`/readyz`、OAuth endpoints 與 `/register` 一律拒絕；`GROUNDLANE_AUTH_TOKEN`、D1 managed token與OAuth access token即使 `principalId=owner` 或帶任意 scope，對 `/admin/credentials` 也一律拒絕。Admin authorization使用exact secret route guard，不依賴一般 `AuthenticatedPrincipal`。
- [x] Managed token create 產生至少 256-bit 高熵 secret，raw secret 只在 create/rotate 時回傳一次；D1 只保存有 indexed public ID 的 verifier/digest 與 bounded metadata。Lookup 先以 public ID 做 indexed single-row query，再 constant-time 比對 verifier；list/audit/log/error/snapshot 不回傳 raw token、verifier、authorization header 或 refresh token。
- [x] Planned rotate的`overlapSeconds`預設3600並只接受整數`0..86400`；negative、overflow、fraction、NaN與錯誤型別在寫入前拒絕。每次建立新credential ID，舊update、新insert與`old.rotatedTo == new.id`/`new.rotatedFrom == old.id`在單一atomic transaction/conditional write完成；lineage維持一對一、無cycle，舊record/verifier不被覆寫或hard delete。
- [x] 新credential原樣繼承principal、scopes/policy與absolute expiry；rotate不能擴權或延長expiry。Authentication只接受`active && now < expiresAt`或`rotating && now < min(validUntil, expiresAt)`，並先拒絕revoked/disabled；production只信任Worker server UTC time，clock boundary、`0` overlap、3600 default與86400 maximum有deterministic fake-clock tests。
- [x] Rotation不需要scheduler/background cleanup；時間前進後，即使D1 stored status仍為`rotating`，下一個request也依`validUntil`拒絕。List output同時提供stored status與derived bounded `effectiveStatus/usable`，不得把已失效credential顯示成仍可用。
- [x] `revoke` 永遠針對指定 credential立即生效並寫入`revokedAt`，不套用 rotation overlap，也不自動撤銷 successor/predecessor。Revoke commit後的新 request被拒絕；已進入執行中的 request不宣稱會被取消。
- [x] Create、list metadata、rotate、revoke、expire、duplicate ID、malformed token、unknown token、disabled token 與 D1 unavailable 都有 deterministic tests；auth storage failure fail closed，不能自動改走一般 static bearer。
- [x] 同一active credential的並行rotate只有一個atomic transition成功，其他request回stable conflict且不產生第二個successor；只有目前active successor可再rotate，舊rotating/revoked/expired record不可重複rotate。ID collision使用bounded retry且不破壞舊record。
- [x] Rotate/revoke race由conditional write/serialization決定：revoke先commit則rotate conflict；rotate先commit後revoke old只撤銷指定old ID，successor不受影響；任何路徑都不得把revoked狀態改回rotating。Revoke具有idempotency，重複request不改寫首次`revokedAt`或產生矛盾lineage。
- [x] Rotate mutation需要bounded idempotency key。Commit成功但response遺失時，相同key只回已建立的新credential ID與`secretAvailable=false`，不重播raw token、不建立第二個successor；CLI不得盲目用新key retry。Operator可明確revoke未知secret record或從該active successor再rotate。
- [x] Rotation/revoke audit至少保存bounded operation ID、admin credential fingerprint/ID、old/new credential IDs、requested overlap、commit timestamp與result，不保存raw token/verifier；list有pagination/output bounds，record不因rotate/revoke hard delete。
- [x] D1 managed-token lookup 不使用 unconstrained read replica；revoke commit 後的新 request 透過 primary/latest session 被拒絕，並有 contract test 與 controlled Cloudflare smoke；KV 不作 managed-token authorization或revocation truth。OAuth 若繼續使用 KV，文件不得宣稱 strong immediate revoke。2026-09-05 controlled smoke：專用 credential 撤銷前 3 次 initialize 成功，direct D1 revoke commit 後 10 次全部 401，正常 control 仍成功且 fixture 已清除；[證據](../verification/managed-revoke-2026-09-05.json)。本次不涵蓋 admin revoke API、多區域或 modern protocol。
- [x] Worker 先移除 caller-supplied internal-auth headers，再只傳 bounded、短效、有 issuer/audience/issued-expiry time、method/path、request ID 與完整性保護的 internal principal context；偽造、過期、錯誤 audience、錯誤 method/path/request binding 與缺少 context 都被 Container 拒絕，外部 raw credential 不跨此 boundary，signing secret 不和 admin/data-plane/provider secrets共用。
- [x] Cloudflare `worker_internal_context` mode拒絕所有 raw bearer fallback；`local_static` mode才接受 legacy `GROUNDLANE_AUTH_TOKEN`，兩種 mode 不可在同一 listener 同時啟用。
- [x] Managed credential 的 `/admin/credentials` API 只接受 `GROUNDLANE_ADMIN_TOKEN`，有 create/list metadata/rotate/revoke/expire contract、request/body bounds、sanitized errors、metadata-only audit 與 deterministic tests；不接受 browser cookie/session auth，不和 OAuth DCR `/register` 共用 route 或語意，也不轉送 Container。
- [x] Admin token不存 D1、不由 admin API rotate；rotation與recovery走 Cloudflare/deployment secret lifecycle。缺少 admin token時 admin API fail closed/unavailable，但既有 data-plane依其 profile繼續服務。
- [x] `groundlane credentials` CLI 只從 protected environment/secret helper讀取 admin token並呼叫 admin API，不持有 D1 binding、database ID/API token或直接執行 SQL；create/rotate raw token只輸出一次，list/revoke輸出不包含 raw token或 verifier。
- [x] Auth matrix deterministic tests覆蓋 admin、legacy、managed、OAuth與缺少 credential對 `/admin/credentials`、`/mcp`、`/readyz`、OAuth endpoints及`/register` 的允許/拒絕結果；`/register` 的 legacy protection只作 OAuth DCR anti-abuse compatibility，不被解讀為 managed-token admin authorization。
- [x] `local_static` profile 沒有 D1、只維持現有單一 static bearer；`worker_internal_context` 的 backward-compatibility deployment 可保留 Worker static/OAuth 驗證，但必須明確回報 managed-token capability unavailable；兩者都不得悄悄建立 multi-static-secret fallback。
...
- [x] Durable job retry runtime 對 provider task creation、付費 upstream call 與 artifact writes 有實際 idempotency/replay integration tests（不只是 contract 驗證）；沒有 provider acknowledgment 時不得宣稱 upstream cancel 成功。實作須符合 `src/core/document-execution.ts` 與 `src/core/corpus-contract.ts` contract 規格。
- [x] D1/DO/Workflow/Queue state runtime 不保存大型內容；durable async `ArtifactRef` 實際接上 storage backend（Cloudflare deployment 映射 R2），包含 tenant/owner binding、content hash、byte size、retention/expiry 與 deletion policy。實作須符合 `src/core/upload-intent.ts` contract 規格。
- [x] Public Web 與 scoped corpus search 實作為不同 MCP tool family；兩者 output 實際區分 source kind、provider/backend、corpus boundary 與 freshness provenance，scoped result 不得標示成全網搜尋結果。實作須符合 `src/core/corpus-contract.ts` contract 規格。
- [x] Corpus lifecycle runtime 實作 create、enroll source、update/resync、remove source、status、search 與 delete，接上 backend adapter。Public corpus/source identity 不使用 backend job/index ID；backend partial failure 映射成 stable state，未完成 deletion 前不宣稱完成。實作須符合 `src/core/corpus-contract.ts` contract 規格。
- [x] Corpus manifest runtime 實際管理 source identity/hash、ACL、retention/deletion policy 與 lifecycle/citation/backend provenance 作為 Groundlane truth source；derived index 可重建且不作為 identity/authorization/retention/deletion truth。實作須符合 `src/core/corpus-contract.ts` contract 規格。
...
| 能力線 | 目前狀態 | 下一步 | Gate |
| --- | --- | --- | --- |
| MCP/control plane | 已有 static/OAuth/D1 managed-token ingress、signed Worker→Container principal context、health/readiness、routing與 tool registry | controlled live revoke smoke、contract versioning、schema snapshot、release checklist | static/OAuth/D1 auth matrix、token lifecycle/revoke consistency、identity spoofing、list-tools、structuredContent、readiness、container failure tests |
...
| Search ownership | public Web routing 已有 provider provenance；獨立 corpus lifecycle／`corpus_search` surface 與 self-host durable SQLite adapter 已接 composition | 補 Cloudflare／managed／external backend adapters與 backend portability eval | corpus identity、source manifest、ACL、enrollment/index-sync state、freshness、retention/deletion、tenant isolation、citation/backend provenance、backend portability |
...
| Parser/document | 已有 HTML parser benchmark；`document_parse` 已掛 deterministic text-PDF、OOXML、structured-text、ODF/RTF/EPUB/EML 與 canonical projection | 擴完整 security/quality fixtures、R2 artifact/cache/async/OCR、tool-surface selection eval | parser unit/contract tests、archive/active/encrypted/deadline bounds、benchmark corpus、cross-client surface A/B/C、production smoke |
...
- MCP Tasks compatibility spike 與 conditional provider slice：先完成 Acceptance criteria 的 MCP `2026-07-28` modern/legacy transport migration，再以正式 Tasks extension 建立最小 provider-owned fixture；對 target clients 驗證 capability negotiation、server-directed task creation、get/update/cancel、terminal result 與斷線續查，保存可重跑 support matrix。不支援 extension 的 client 使用明確 start/status/result/cancel tools。短 research 維持同步；只有 client support gate 通過後，才以一個 provider-owned task 掛第一個 async research slice，並補齊 ownership、expiry、credential binding、billing provenance與三層 cancellation evidence。
...
- DocumentSource tagged input 與 bounded inline/public-URL runtime 已實作；完整設定的 Cloudflare edge profile 也已接上 D1/R2 reference adapter、短效 SigV4 single-PUT handoff、verify/finalize 與 body-bound processing bridge。這條路徑目前只有 deterministic fake-D1/R2 與 local signed-bridge 證據，尚未完成 controlled deployment、真實 R2 與 target-client smoke。Large/private document upload 的已核准需求構成 R2 adapter 的專屬 adoption gate，不因此核准 R2 承擔 corpus truth、state machine 或其他 stateful responsibilities。
- Artifact retention 與 processing cache 採 tiered defaults：upload intent 15 分鐘、staging cleanup window一小時、transient `ArtifactRef` 24 小時、processing cache 24 小時；caller 可在 deployment-advertised bounds 內調整，operator 可改 defaults/max 或停用 cache。Upload/artifact runtime 已採前述 intent/artifact defaults，但 operator-configurable bounds、scheduled physical cleanup、成本、target-client retry 與 privacy fixtures仍待驗證。Corpus enrollment 預設保留到 explicit remove/delete，也可由 corpus/source policy 設 expiry。
...
- Groundlane-owned corpus lifecycle：operator-owned knowledge access 與 corpus control-plane ownership 已確認屬於 Groundlane 的產品方向；corpus create/enroll/update/remove/status/delete 與 `corpus_search` 已有 self-host durable SQLite adapter-backed slice。Groundlane 負責 corpus identity、source manifest、ACL、enrollment/index-sync state、freshness、retention/deletion、tenant isolation、citation 與 backend provenance contract；SQLite derived index、未來的 Cloudflare AI Search／Vectorize、外部 provider 或自架 index都只作可替換 backend adapter，不得接管 public contract，也不改變 public Web `web_search` 的語意。Cloudflare／managed backend仍需具體需求、evaluation corpus、backend portability/deletion proof與 isolation gate；Groundlane-native ranking engine也需另立需求與品質 gate。
