...
- `web_search(query, maxResults, domains, excludeDomains, timeRange, provider, timeoutMs)`：先代理 Tavily／Exa／Parallel／Browserbase／Brave／Firecrawl／SerpApi；只公開已確認有每月循環免費額度的 provider，不自建索引。
