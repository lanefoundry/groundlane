...
`document_parse` is the bounded synchronous file/document tool. Its `source` is either canonical base64 inline bytes, a public URL protected by the existing fetch/SSRF policy, or a verified source `ArtifactRef`. The current composition supports inline and URL sources; artifact input returns `PROVIDER_UNAVAILABLE` until a durable artifact reader is configured. `output` selects `markdown` (default), `structured`, `text`, or `all`; `maxBytes`, `maxPages`, `maxOutputChars`, and `timeoutMs` only narrow operator limits. Set `DOCUMENT_CACHE_STATE_PATH` to a writable SQLite file to enable the self-hosted ownership-scoped processing cache. `cacheMode=use|refresh|bypass` defaults to `use`; `cacheTtlSeconds` must be between 60 seconds and the configured `DOCUMENT_CACHE_MAX_TTL_SECONDS`, otherwise the request is rejected. Hits reuse only content-addressed parsed payload; each response rebuilds source identity and the canonical envelope for the current invocation. URL bindings distinguish requested and final URLs. Cache failures fail open to fresh parsing and expose only a stable sanitized cache error. Current deterministic profiles cover text PDF, DOCX/XLSX/PPTX, CSV/TXT/Markdown/JSON/XML/HTML, and bounded ODF/RTF/EPUB/EML. OCR, async execution, Cloudflare D1 cache composition, and R2-backed artifact input are not active runtime paths.
