...
| Variable | Purpose | Default/example |
| --- | --- | --- |
| `OCR_SPACE_API_KEY` | OCR.space API key for `document_ocr` (scanned PDFs, images) | Optional; free 25,000 requests/month at [ocr.space/OCRAPI](https://ocr.space/OCRAPI); tool fails closed without it |
| `CLOUDCONVERT_API_KEY` | CloudConvert API key for `document_convert` (`.doc`→`.docx`, `.xls`→`.xlsx`, `.ppt`→`.pptx`) | Optional; free 25 conversions/day; tool fails closed without it |

`document_ocr` supports PDF, PNG, JPEG, GIF, TIFF, BMP, and WebP inputs up to 1 MB. `document_convert` converts legacy Office formats to their modern equivalents; the converted output can then be fed to `document_parse`. Both tools are registered in Full and Lite compositions and return an explicit configuration error when their credential is missing.

`document_transcribe` uses Cloudflare Workers AI (Whisper) for audio-to-text transcription. It reuses `CF_BROWSER_ACCOUNT_ID` and `CF_BROWSER_API_TOKEN` from the browser backend section and requires no additional credential. Supported formats: MP3, WAV, WebM, OGG, FLAC, M4A up to 25 MB. Free within Workers AI daily Neuron allowance.

`document_parse`, `document_table_extract`, `document_archive_extract`, `document_email_extract`, `document_chunk`, and `document_toc` are fully deterministic local tools that require no credentials or external API calls.
