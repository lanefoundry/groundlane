...
| SerpApi | present | 250 | OK, 1 result | Unknown in Groundlane | Account API found at `GET https://serpapi.com/account.json`; no Groundlane checker yet |
...
| Firecrawl | present | 500 | Upstream rejected request | Unknown in Groundlane | Credit Usage API found at `GET https://api.firecrawl.dev/v2/team/credit-usage`; no Groundlane checker yet |
...
| Linkup | Official docs state eligible accounts receive `$20` credits and expose `GET /v1/credits/balance` | `13.363` credits from production `provider_balance` on 2026-08-29 after live answer smoke | Search, Fetch, Research, Tasks, Extract, source/domain filtering | `web_search` keyed REST; `web_answer` via `outputType=sourcedAnswer`; `web_research` via async Research task polling; `web_content` via Fetch; `provider_balance`; `provider_capabilities` | Balance response shape verified from official docs as `{ balance: number }`; Research can exceed Groundlane's synchronous MCP deadline |
...
| SerpApi | Prior official pricing snapshot: 250 searches/month free | Unknown | Google and vertical SERP APIs | `web_search` keyed Google organic adapter; `web_news` via Google News; `web_images` via Google Images | Account API is a future `provider_balance` candidate; Groundlane normalizes organic web, Google News, and Google Images results today |
...
| Firecrawl | Prior official pricing snapshot: 1,000 credits/month | Unknown | Search, Scrape, Crawl, Map, Extract, hosted MCP | `web_search` keyed REST; `web_content` via Scrape; `web_map` via Map; `web_crawl` via Crawl | Credit Usage API is a future `provider_balance` candidate; production `web_content(provider=firecrawl)` on 2026-08-29 returned sanitized request rejection |
...
2. Add Firecrawl and SerpApi balance checkers: both have official account-level
   balance or remaining-usage endpoints.
3. Evaluate Parallel balance separately because it requires an Account API OAuth
   bearer token rather than the standard API key used by search/research calls.
4. Keep Exa and Browserbase as usage/cost diagnostics, not provider balance:
   their checked APIs report usage analytics rather than remaining credits.
5. Keep Serper, Brave, Tavily, and Keenable balance as dashboard-only or
   unclear unless an official account-level API is found.
6. Do not infer real provider quota from Groundlane local budgets or live smoke
...
| Firecrawl | Yes, account remaining credits | `GET https://api.firecrawl.dev/v2/team/credit-usage` | `remainingCredits`, `planCredits`, billing period | Add `provider_balance` checker | `https://docs.firecrawl.dev/api-reference/endpoint/credit-usage` |
| SerpApi | Yes, account usage and remaining searches | `GET https://serpapi.com/account.json` | searches used/left, extra credits, hourly limit | Add `provider_balance` checker | `https://serpapi.com/account-api` |
