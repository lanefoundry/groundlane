...
| SerpApi | present | 250 | OK, 1 result | Unknown until production `provider_balance` is refreshed after deployment | Implemented in `provider_balance` through `GET https://serpapi.com/account.json`, reports total searches left |
...
| Firecrawl | present | 500 | Upstream rejected request | Unknown until production `provider_balance` is refreshed after deployment | Implemented in `provider_balance` through `GET https://api.firecrawl.dev/v2/team/credit-usage`, reports remaining credits |
...
| SerpApi | Prior official pricing snapshot: 250 searches/month free | Unknown until production `provider_balance` is refreshed after deployment | Google and vertical SERP APIs | `web_search` keyed Google organic adapter; `web_news` via Google News; `web_images` via Google Images; `provider_balance`; `provider_capabilities` | Account API is implemented in `provider_balance`; Groundlane normalizes organic web, Google News, and Google Images results today |
...
| Firecrawl | Prior official pricing snapshot: 1,000 credits/month | Unknown until production `provider_balance` is refreshed after deployment | Search, Scrape, Crawl, Map, Extract, hosted MCP | `web_search` keyed REST; `web_content` via Scrape; `web_map` via Map; `web_crawl` via Crawl; `provider_balance`; `provider_capabilities` | Credit Usage API is implemented in `provider_balance`; production `web_content(provider=firecrawl)` on 2026-08-29 returned sanitized request rejection |
...
1. Run production `provider_balance` after deployment to refresh Linkup,
   You.com, Firecrawl, and SerpApi balances from their official APIs.
2. Evaluate Parallel balance separately because it requires an Account API OAuth
   bearer token rather than the standard API key used by search/research calls.
3. Keep Exa and Browserbase as usage/cost diagnostics, not provider balance:
   their checked APIs report usage analytics rather than remaining credits.
4. Keep Serper, Brave, Tavily, and Keenable balance as dashboard-only or
   unclear unless an official account-level API is found.
5. Do not infer real provider quota from Groundlane local budgets or live smoke
