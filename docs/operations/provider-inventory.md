...
| TinyFish | not verified | 3000 | Not smoke-tested | Search and Fetch are free at any wallet balance; limits are 30 search requests/min and 150 fetch URLs/min | Usage endpoint exists for Fetch history; no account-balance endpoint found in reviewed docs |
...
| SearchAPI.io | not verified | 0 | Not smoke-tested | Actual remaining requests unknown; pricing page advertises 100 free signup requests | No public balance endpoint found in reviewed SearchAPI.io docs |
...
| TinyFish | Official docs: Search and Fetch are free at any wallet balance; pricing page lists 30 requests/min for Search and 150 URLs/min for Fetch | Unknown | Search, Fetch, Agent, Browser, MCP, SDKs, CLI | `web_search` keyed REST; `web_content` via Fetch; `provider_capabilities` | Account still needs API access and `TINYFISH_API_KEY`; Agent and Browser are paid surfaces and are not exposed |
...
| SearchAPI.io | Pricing page advertises 100 free signup requests | Unknown | Google and vertical SERP APIs | `web_search` keyed Google organic adapter; `provider_capabilities` | Kept opt-in with default budget `0` because the free pool is a finite signup trial |
...
`provider_quota` and `search_budget_status` apply to every provider in this
matrix because they report Groundlane diagnostics. `provider_quota` also
...
| TinyFish | `web_search` and `web_content` provider | Search, Fetch, Agent, Browser, MCP, SDKs, CLI | include domains, exclude domains, date range through `recency_minutes` |
...
4. Use `provider_quota` as the first diagnostic view for zero-result search
   runs or exhausted provider-backed tool attempts. It combines provider
   account-balance status, Groundlane's local provider-dispatch guardrails,
   capability data, and `searchRouting` hints while keeping those concepts in
...
| TinyFish | Fetch usage history only | `GET https://api.fetch.tinyfish.ai/usage` | fetch history, status, text length, latency | Future `provider_usage`, not balance | `https://docs.tinyfish.ai/fetch-api/reference` |
...
| SearchAPI.io | Dashboard/pricing only in checked docs | none found | requests in dashboard | Dashboard-only until official API is found | `https://www.searchapi.io/pricing` |
...
| TinyFish | `https://docs.tinyfish.ai/search-api/reference`, `https://docs.tinyfish.ai/fetch-api/reference` | Agent, Browser, Fetch usage history, selectors, conditional fetch, research-paper search | richer `web_content` options, provider usage diagnostics |
...
| SearchAPI.io | `https://www.searchapi.io/docs/google` | Many vertical Google engines beyond organic web | `web_places`, `web_shopping`, `web_scholar` |
