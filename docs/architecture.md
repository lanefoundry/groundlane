...
Durable document work has an explicit storage boundary. `DurableRecordStorePort` stores at most 64 KiB of revision-fenced metadata and has SQLite and D1 adapters; `ImmutableBlobPort` stores content-addressed bytes and has SQLite and R2 adapters. The self-hosted Node composition can mount `DurableDocumentCacheRepository` through `DOCUMENT_CACHE_STATE_PATH`; content metadata/source bindings and immutable parsed-payload blobs survive restart, remain ownership-scoped, and preserve source-specific response provenance. The Cloudflare profile composes the same callback-free lookup/commit contract over D1/R2 through a signed private Container outbound handler, while parsing stays inside the Container and cache policy stays inside the Worker. Credential-scoped bindings, artifact-expiry caps, CAS fences, and bounded blob-before-metadata cleanup have deterministic tests. Durable job and artifact repositories plus the side-effect journal have restart/race tests. When `CORPUS_STATE_PATH` is set, the self-hosted composition mounts a revision-fenced corpus manifest repository, immutable normalized source artifacts, and a rebuildable SQLite derived index; lifecycle, credential/tenant binding, ACL, expiry, restart, exact rebuild, removal, and retryable deletion have integration coverage. The Cloudflare source-upload path composes D1 metadata, R2 staging/final bytes, and a separate signed parser bridge; result artifacts, async document jobs, a Cloudflare corpus adapter, public deletion-to-cache revocation, and controlled deployment proof remain pending. Managed-authorization reads require a `first-primary` D1 session and fail closed when Sessions are unavailable.
